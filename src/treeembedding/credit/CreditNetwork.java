...
public class CreditNetwork extends Metric {
    ...
    boolean shortest; //using multi-party computation to determine minimum or do routing adhoc
    boolean disjoint; //using multi-party computation to determine minimum or do routing adhoc
    ...
    // routeSuccPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\Results\\routeSucc-t0-graph-root3-dyn-ChangeCredit21-2022-01-06-Test1-OriginalWeight.txt";
    //String paySuccPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\Results\\paySucc-t0-graph-root3-dyn-ChangeCredit21-2022-01-06-Test1-OriginalWeight.txt";
//    String routeSuccPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\Results\\Mydataset-routeSucc-t0-graph-root3-dyn-test21.txt";
    String routeSuccPath = "X:/out/Mydataset-routeSucc-t0-graph-root3-dyn-test21.txt";
    //    String paySuccPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\Results\\Mydataset-paySucc-t0-graph-root3-dyn-test21.txt";
    String paySuccPath = "X:/out/Mydataset-paySucc-t0-graph-root3-dyn-test21.txt";
    String edgeWeightPath = "X:/out/edgeWeigt.txt";
    String paySuccessAllPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\paySuccessPath-t0-graph-root3-dyn-1.txt";
    String routeSuccessAllPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\routeSuccessPath-t0-graph-root3-dyn-1.txt";
    String routeFailureAllPath = "D:\\code\\speedymurmur-experiments\\speedy\\data\\routeFailurePath-t0-graph-root3-dyn-1.txt";
    String routeFailPath = "X:\\out\\Mydataset-routeFail-t0-graph-root3-dyn-test21.txt";
...
//    Long routeFail = 0L;
    ...
    public CreditNetwork(String file, String name, double epoch, Treeroute ra, boolean dynRep,
                         boolean multi, double requeueInt, Partitioner part, int[] roots,
                         int max, String links, boolean up) {
    ...
    public CreditNetwork(String file, String name, double epoch, Treeroute ra, boolean dynRep,
        ...
        this(file, name, epoch, ra, dynRep, multi, requeueInt, part, roots, max, links, true);// up is true is changed to false by anan.
        // up is true that means not update the weights and change weights with transfer. Dynamic
        // up is false that means update the weights and not change weights withe transfer.  Static. reset to original state evey time
    }
    ...
    public void computeData(Graph g, Network n, HashMap<String, Metric> m) {
        ...
        int allTransaction = 0;// transaction number @add by anan
        int topupNumbers = 0;// topup times @add by anan
        int FailTimes = 0; // Fial transaction times @add by anan
        ...
        int c = 0;
        double cur_succ = 0;
        int cur_count = 0; // record the transaction number
        Random rand = new Random(1);//modify by Anan
        ...
        while (c < this.transactions.size() || toRetry.size() > 0) {
            ...
            if (!this.newLinks.isEmpty()) {
                double nt = this.newLinks.peek()[0];// get transaction time of newlinks
                while (nt <= cur.time) {// judge if newlinks transactions time < current transaction time
            ...
            allTransaction++;
            System.out.println("The current transaction number is: " + allTransaction);
            ...
            if (epoch_cur != epoch_old) {
                if (!this.dynRepair) {
                    ...
                    for (int j = epoch_old + 1; j <= epoch_cur; j++) {
                        ...
                        System.out.println("The edgeweights is" + edgeweights);// Add by anan
                    }
                } else {
            ...
            if (this.multi) {
                ...
                //System.out.println("The edgeweights 1 is:"+edgeweights.getWeights().toString());// Add by anan
            } else if (this.shortest) {
                results = this.routeShortPath(cur, g, nodes, exclude, edgeweights);
            } else if (this.disjoint) {
                results = this.routeDisjointPath(cur, g, nodes, exclude, edgeweights);
            } else {
                results = this.routeAdhoc(cur, g, nodes, exclude, edgeweights);
                //System.out.println("The edgeweights 2 is:"+edgeweights.getWeights().toString());// Add by anan
            }

            //System.out.println("The results is"+results[0]);// Add by anan
            if (results[0] == 0) {
                FailTimes++;
                //System.out.println("Fail times is:" + FailTimes);
            }

            //reset to old weights if failed
            if (results[0] == -1) { //results[0]=0 is success and results[0]=-1 is failure
                System.out.println("1-----This is update weight to function------1");

                this.weightUpdate(edgeweights, originalWeight); //update edgeweight by add 10 every time @anan write

                topupNumbers++;
//				System.out.println("The value is:"+cur.val);
//				System.out.println("The path is:"+cur.path);
                //System.out.println("Top up 10 per time and topup times is:"+topupNumbers);
            ...
            if (!this.update) {
                ...
                System.out.println("2-----Update weight----2");
            }
        }
...
//            WriteToTxt.writeTxt(edgeWeightPath, s);
        ...
        this.flag1 = this.flag1 / 1000;
        this.flag2 = this.flag2 / 10000;
        this.flag3 = this.flag3 / 20000;
        this.flag4 = this.flag4 / 30000;
        this.flag5 = this.flag5 / 40000;
        this.flag6 = this.flag6 / 50000;
        this.flag7 = this.flag7 / 60000;
        this.flag8 = this.flag8 / 70000;
        this.flag9 = this.flag9 / 80000;
        this.flag10 = this.flag10 / 90000;
...
//                    w = edgeweights.getPot(l, k);
    ...
    private int[] routeAdhoc(Transaction cur, Graph g, Node[] nodes, boolean[] exclude,
                             CreditLinks edgeweights) {
        int[][] paths = new int[roots.length][];
        int src = cur.src;
        int dest = cur.dst;
        //distribute values on paths
        double[] vals = this.part.partition(g, src, dest, cur.val, roots.length);//Split the value to every landmarks to three parts, all money transafer by different routes.
...
//                paths[j] = this.ra.getRoute(s, d, j, g, nodes, exclude, edgeweights, vals[j], false);

                String path = ""; // add by me
                for (int i = 0; i < paths[j].length; i++) {
                    path = path + "->" + paths[j][i];
                    //String SingleEdge = path;
                    //WriteToTxt.writeEdgetoTxt(SingleEdge);
                }
                ...
                if (paths[j][paths[j].length - 1] == -1) {
                    succ = false;
                    //String FailureEdge = Integer.toString(dest);
                    //String AllEdges = "Alledges failure" + path + "->" + FailureEdge;
                    //WriteToTxt.writeEdgetoTxt(AllEdges);
                    //WriteToTxt.writeTxt(routeSuccessAllPath, path);

                } else {
                    // String AllEdges = "Alledges Success" + path;
                    // WriteToTxt.writeEdgetoTxt(AllEdges);
                    ...
                    for (int i = 1; i < paths[j].length; i++) {
                        int k = paths[j][i];
                        Edge e = edgeweights.makeEdge(l, k);
                        Edge old_e = edgeweights.makeEdge(k, l);
                        System.out.println("The previous balance of (" + l + "," + k + ") is " + edgeweights.getPot(l, k));// print the previous balance of two nodes
                        System.out.println("The previous balance of (" + k + "," + l + ") is " + edgeweights.getPot(k, l));// print the previous balance of two nodes
                        double w = edgeweights.getWeight(e);
                        double old_w = edgeweights.getWeight(old_e);
                        //System.out.println("The previous weight of (" + l + "," + k + ") is " + w);// print the previous weight of two nodes
                        //System.out.println("The previous weight of (" + k + "," + l + ") is " + old_w);// print the previous balance of two nodes
                        if (!originalWeight.containsKey(e)) {
                            originalWeight.put(e, w);
                        }
                        // Increase the transfered value (vals[j]) between the nodes l and k
                        edgeweights.setWeight(l, k, vals[j]);
                        //if (log){
                        System.out.println("----Set weight of (" + l + "," + k + ") to " + edgeweights.getWeight(e)
                                + " (previous " + w + ")");
                        System.out.println("The new balance of (" + l + "," + k + ") is " + edgeweights.getPot(l, k));// print the new balance of two nodes
                        System.out.println("The new balance of (" + k + "," + l + ") is " + edgeweights.getPot(k, l));// print the new balance of two nodes
                        //	}
                        if (edgeweights.getPot(l, k) == 0) {//calculate the balance between two nodes
                            this.zeroEdges.add(e);
                        }
                        //System.out.println("The new weight of (" + l + "," + k + ") is " + edgeweights.getWeight(e));// print the new balance of two nodes
                        // System.out.println("The new weight of (" + k + "," + l + ") is " + edgeweights.getWeight(old_e));// print the new balance of two nodes
                        l = k;
                    }
                }
            }
        ...
        if (flag == 0) {
            ...
            for (int j = 0; j < paths.length; j++) {
                ...
                if (paths[j][paths[j].length - 1] == -1) {
                    ...
                    // System.out.println("The depth of " + paths[j][paths[j].length - 2] + " is:" + depth);
                    for (int k = 0; k < paths[j].length - 2; k++) {
                        path = path + "->" + paths[j][k];
                        routeSucc = edgeweights.getTimes(paths[j][k], paths[j][k + 1], routeSucc);
                    }
        ...
        if (!succ) {
            //if (log){ // cancel if (log) to print it
            System.out.println("---Failure---2");
            String s = cur.time + " " + cur.val + " " + cur.src + " " + cur.dst;
            //String s = cur.val + " " + cur.src + " " + cur.dst;

            PaymentAmount += cur.val;
            // WriteToTxt.writeFailTxstoTXT(s);
            //.writeTxt(s);
            ...
            PaymentAmount += cur.val;
            // Write the success transaction to txt file.
            // String s = cur.time + " " + cur.val + " " + cur.src + " " + cur.dst;
            //WriteToTxt.writeTxt(s);
//            System.out.println("" + cur.time + " " + cur.src + " " + cur.dst + " " + cur.val);
            //}
        }
    ...
    private void weightUpdate(CreditLinks edgeweights, HashMap<Edge, Double> updateWeight) {
        ...
        while (it.hasNext()) {
            ...
            int src = entry.getKey().getSrc();// add by me
            int dst = entry.getKey().getDst();// add by me

            System.out.println("The Current Balances is (" + src + "->" + dst + "): " + edgeweights.getPot(src, dst));

            edgeweights.setWeight(entry.getKey(), entry.getValue());

            Double Balance = edgeweights.getPot(src, dst);
            // String s = src + " " + dst + " " + Balance;
            // WriteToTxt.writeFailTxBalancetoTxt(s);
            System.out.println("The Final Balances is (" + src + "->" + dst + "): " + edgeweights.getPot(src, dst));
            //edgeweights.setWeight(entry.getKey(), entry.getValue()*100);
            //edgeweights.setWeight(entry.getKey(), toptoken);// add by me
            //System.out.println("Top up token is:"+toptoken);
            //System.out.println("Top up 10");
        }
    }
    ...
    public Single[] getSingles() {
        ...
        Single sf1 = new Single("FLAG_SUCCESS_0_1000", this.flag1);
        Single sf2 = new Single("FLAG_SUCCESS_0_10000", this.flag2);
        Single sf3 = new Single("FLAG_SUCCESS_0_20000", this.flag3);
        Single sf4 = new Single("FLAG_SUCCESS_0_30000", this.flag4);
        Single sf5 = new Single("FLAG_SUCCESS_0_40000", this.flag5);
        Single sf6 = new Single("FLAG_SUCCESS_0_50000", this.flag6);
        Single sf7 = new Single("FLAG_SUCCESS_0_60000", this.flag7);
        Single sf8 = new Single("FLAG_SUCCESS_0_70000", this.flag8);
        Single sf9 = new Single("FLAG_SUCCESS_0_80000", this.flag9);
        Single sf10 = new Single("FLAG_SUCCESS_0_90000", this.flag10);

        Single tp1 = new Single("THROUGHPUT_0_1000", this.throughput1);
        Single tp2 = new Single("THROUGHPUT_0_10000", this.throughput2);
        Single tp3 = new Single("THROUGHPUT_0_20000", this.throughput3);
        Single tp4 = new Single("THROUGHPUT_0_30000", this.throughput4);
        Single tp5 = new Single("THROUGHPUT_0_40000", this.throughput5);
        Single tp6 = new Single("THROUGHPUT_0_50000", this.throughput6);
        Single tp7 = new Single("THROUGHPUT_0_60000", this.throughput7);
        Single tp8 = new Single("THROUGHPUT_0_70000", this.throughput8);
        Single tp9 = new Single("THROUGHPUT_0_80000", this.throughput9);
        Single tp10 = new Single("THROUGHPUT_0_90000", this.throughput10);
        ...
        return new Single[]{m_av, m_Re_av, m_S_av, m_F_av, p_av, p_Re_av, p_S_av, p_F_av,
                ...
                tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, pP_av, pPF_av, pPNF_av, rt, d1, d2, d3};
    }
