...
//每隔stepcount条交易调用一次的
public class CreditFlare2 extends Metric {
    //input parameters
    ...
    double FailSuccessAmount = 0;
    double FailSuccessCount = 0;
    double FailPaymentAmount = 0;

    List SuccessAmountList = new ArrayList<>();
    List SuccessCountList = new ArrayList<>();
    List PaymentAmountList = new ArrayList<>();

    List FailSuccessAmountList = new ArrayList<>();
    List FailSuccessCountList = new ArrayList<>();
    List FailPaymentAmountList = new ArrayList<>();
    ...
    //    String routeSuccPath = "X:/out/jan2013-lcc-t0-FP-routeSucc-root1-dyn-1-NoSP.txt";
//    String paySuccPath = "X:/out/jan2013-lcc-t0-FP-paySucc-root1-dyn-1-NoSP.txt";
//    String edgeWeightPath = "X:/out/jan2013-lcc-t0-FP-finished-transactions-Weight-root1-dyn-1-NoSP.txt";
//    String routeFailPath = "X:/out/jan2013-lcc-t0-FP-routeFail-root1-dyn-1-NoSP.txt";
//    String routeSuccPath = "D:\\code\\PaySwitch\\data\\Results\\jan2013-lcc-t0-FP-routeSucc-root1-dyn-1-SP-E8-All.txt";
//    String paySuccPath = "D:\\code\\PaySwitch\\data\\Results\\jan2013-lcc-t0-FP-paySucc-root1-dyn-1-SP-E8-All.txt";
//    String edgeWeightPath = "D:\\code\\PaySwitch\\data\\Results\\jan2013-lcc-t0-FP-finished-transactions-Weight-root1-dyn-1-SP-E8-All.txt";
//    String routeFailPath = "D:\\code\\PaySwitch\\data\\Results\\jan2013-lcc-t0-FP-routeFail-root1-dyn-1-SPE8-All.txt";

    String routeSuccPath = "./data/Results/jan2013-lcc-t0-FP-routeSucc-root1-dyn-1-SP-E8-All.txt";
    String paySuccPath = "./data/Results/jan2013-lcc-t0-FP-paySucc-root1-dyn-1-SP-E8-All.txt";
    String edgeWeightPath = "./data/Results/jan2013-lcc-t0-FP-finished-transactions-Weight-root1-dyn-1-SP-E8-All.txt";
    String routeFailPath = "./data/Results/jan2013-lcc-t0-FP-routeFail-root1-dyn-1-SPE8-All.txt";

    Map<Edge, Long> routeSucc = new HashMap<>();
    Map<Edge, Long> paySucc = new HashMap<>();
    Map<Edge, Long> routeFail = new HashMap<>();

    public CreditFlare2(String file, String name, int rn, int k, int nb, int ntab,
                        double epoch, Partitioner part, String links, boolean up, int stepCount) {
    ...
    public CreditFlare2(String file, String name, int rn, int k, int nb, int ntab,
                        double epoch, Partitioner part, boolean up) {
        this(file, name, rn, k, nb, ntab, epoch, part, null, up);
    }

    public CreditFlare2(String file, String name, int rn, int k, int nb, int ntab,
                        double epoch, Partitioner part, boolean up, int stepCount) {
        this(file, name, rn, k, nb, ntab, epoch, part, null, up, stepCount);
    }
    ...
    public void computeData(Graph g, Network n, HashMap<String, Metric> m) {
        ...
        Queue<Transaction> failTrans = new LinkedList<Transaction>();
        int epoch_old = 0;
        int epoch_cur = 0;
        int cur_stab = 0;
        int c = 0;
        ...
        while (c < this.transactions.size() || toRetry.size() > 0) {
            ...
            count++;
            System.out.println("current transaction number: " + count);
            //if (log){
            ...
            int[] results = this.route(cur, g, nodes, exclude, false);
            ...
            if (count % stepCount == 0) {
                while (failTrans.size() > 0) {
                    ...
                    failCount++;
                    System.out.println("current transaction number: " + count);
                    System.out.println("Perform transaction s=" + cur.src + " d= " + cur.dst + " val= " + cur.val + " time= " + cur.time + " " + (new Date()));
                    //2: execute the transaction
                    originalWeight = new HashMap<Edge, Double>();
                    results = this.route(cur, g, nodes, exclude, true);
                    //re-queue if necessary
                    cur.addPath(results[1]);
                    cur.addMes(results[4]);
                    //reset to old weights if failed
                    ...
                    FailPaymentAmount += cur.val;
                    if (failCount % stepCount == 0) {
                        FailSuccessAmountList.add(FailSuccessAmount);
                        FailPaymentAmountList.add(FailPaymentAmount);
                        FailSuccessCountList.add(FailSuccessCount);
                    }
                    for (int j = 6; j < results.length; j++) {
                        int index = 0;
                        if (results[j] < 0) {
        ...
        /*FailSuccessAmountArray = new double[(failTrans.size() / stepCount) + 1];
        FailPaymentAmountArray = new double[(failTrans.size() / stepCount) + 1];
        FailSuccessCountArray = new double[(failTrans.size() / stepCount) + 1];
        int failCount = 0;
        while (failTrans.size() > 0) {
            ...
            //2: execute the transaction
            originalWeight = new HashMap<Edge, Double>();
            int[] results = this.route(cur, g, nodes, exclude, true);
            //re-queue if necessary
            cur.addPath(results[1]);
            cur.addMes(results[4]);
            //reset to old weights if failed
            ...
            FailPaymentAmount += cur.val;
            if (failCount % stepCount == 0) {
                FailSuccessAmountArray[failCount / stepCount - 1] = FailSuccessAmount;
                FailPaymentAmountArray[failCount / stepCount - 1] = FailPaymentAmount;
                FailSuccessCountArray[failCount / stepCount - 1] = FailSuccessCount;
            }
        ...
        FailSuccessAmountArray[failCount / stepCount] = FailSuccessAmount;
        FailPaymentAmountArray[failCount / stepCount] = FailPaymentAmount;
        FailSuccessCountArray[failCount / stepCount] = FailSuccessCount;*/
...
//            WriteToTxt.writeTxt(edgeWeightPath, s);
     ...
     */
    private int[] route(Transaction cur, Graph g, Node[] nodes, boolean[] exclude, boolean switchpay) {
        int rec = cur.dst;
        CreditLinks edgeweights = (CreditLinks) g.getProperty("CREDIT_LINKS");
        ...
        for (int j = 0; j < paths.size(); j++) {
            ...
            for (int i = 1; i < p.length; i++) {
                ...
                if (switchpay) {
                    if (cur.val > w) {
                        SwitchPayment.switchPayment(p[i - 1], p[i], nodes, edgeweights, cur.val);
                    }
                    w = edgeweights.getPot(p[i - 1], p[i]);
                }
                if (w < mins[j]) {
                    mins[j] = w;
                }
        ...
        if (succ) {
            int[] p = paths.get(0);
            for (int i = 1; i < p.length; i++) {
                paySucc = edgeweights.getTimes(p[i - 1], p[i], paySucc);
            }
        } else {
            if (paths.size() != 0) {
                int[] p = paths.get(0);
                for (int i = 1; i < p.length; i++) {
                    routeSucc = edgeweights.getTimes(p[i - 1], p[i], routeSucc);
                }
            }
        }
    ...
    public Single[] getSingles() {
        ...
        Single SAA = new Single("SuccessAmountArray", 0L, ListToDouble(SuccessAmountList));
        Single SCA = new Single("SuccessCountArray", 0L, ListToDouble(SuccessCountList));
        Single PAA = new Single("PaymentAmountArray", 0L, ListToDouble(PaymentAmountList));

        Single FSAA = new Single("FailSuccessAmountArray", 0L, ListToDouble(FailSuccessAmountList));
        Single FSCA = new Single("FailSuccessCountArray", 0L, ListToDouble(FailSuccessCountList));
        Single FPAA = new Single("FailPaymentAmountArray", 0L, ListToDouble(FailPaymentAmountList));

        return new Single[]{m_av, m_S_av, m_F_av, p_av, p_S_av, p_F_av, d1, d2, d3,
                s_av, s, SAA, SCA, PAA, FSAA, FSCA, FPAA, pP_av, pPF_av, pPNF_av, init};
    }
