...
//失败链路达到10*N后进行调用
public class CreditMaxFlow3 extends Metric {
    // input parameters
...
//	int failCount = 0;
//	int stepCount = 100;
//	double[] failThroughput = new double[100000 / stepCount];
    ...
    public CreditMaxFlow3(String file, String name, double requeueInt,
    ...
    public CreditMaxFlow3(String file, String name, double requeueInt,
                          int max, String links, boolean up, double epoch, int edgeFailCount) {
        super("CREDIT_MAX_FLOW", new Parameter[]{
                new DoubleParameter("REQUEUE_INTERVAL", requeueInt),
                new IntParameter("MAX_TRIES", max)});
        transactions = this.readList(file);
    ...
    public CreditMaxFlow3(String file, String name, double requeueInt, int max,
                          String links, double epoch, int edgeFailCount) {
        this(file, name, requeueInt, max, links, true, edgeFailCount);
    }
    ...
    public CreditMaxFlow3(String file, String name, double requeueInt, int max, double epoch) {
    ...
    private int[] fordFulkerson(Transaction cur, Graph g, Node[] nodes, boolean[] exclude) {
        ...
        while (totalflow < cur.val && (resp = findResidualFlow(edgeweights, g.getNodes(), src, dest)).length > 1) {
            if (log)
                System.out.println("Found residual flow " + resp[0].length);
            ...
            for (int i = 0; i < respath.length; i++) {
                path_Anan = path_Anan + "->" + respath[i];
            }
            System.out.println("The path is:" + path_Anan);

            for (int i = 0; i < respath.length - 1; i++) {
                double a = edgeweights.getPot(respath[i], respath[i + 1]);
                // call Switch payment
                if (cur.val > a) { // add by @anan start
                    Edge edge = new Edge(respath[i], respath[i + 1]);
                ...
                a = edgeweights.getPot(respath[i], respath[i + 1]); // add by @anan end
            ...
            for (int i = 0; i < respath.length - 1; i++) {
                ...
                if (n1 < n2) {
                    ...
                    //if (log)
                    System.out.println("Set weight of (" + n1 + "," + n2 + ") to " + (w + min));
                } else {
                    edgeweights.setWeight(e, w - min);
                    //if (log)
                    System.out.println("Set weight of (" + n2 + "," + n1 + ") to " + (w - min));
                }
            }
            mes = mes + resp[1][0];
            ...
            // record the each type path @anan
            if (cur.val - totalflow > 0) { //transaction fail
                for (int j = 0; j < respath.length - 1; j++) {
                    if (cur.val <= edgeweights.getPot(respath[j], respath[j + 1])) {
                        routeSucc = edgeweights.getTimes(respath[j], respath[j + 1], routeSucc);
                    } else {
                        routeFail = edgeweights.getTimes(respath[j], respath[j + 1], routeFail);
                    }
                }
            } else { //transaction success
                for (int j = 0; j < respath.length - 1; j++) {
                    paySucc = edgeweights.getTimes(respath[j], respath[j + 1], paySucc);
                }
            }
            //end record
        }
    ...
    public Single[] getSingles() {
        ...
        return new Single[]{m_av, m_Re_av, m_S_av, m_F_av, p_av, p_Re_av, p_S_av, p_F_av,
                s1, s, SAA, SCA, PAA, pP_av, pPF_av, pPNF_av};
    }
