...
//失败链路值大于N后进行调用
public class CreditNetwork5 extends Metric {
    // input parameters
    ...
    public CreditNetwork5(String file, String name, double epoch, Treeroute ra, boolean dynRep,
        ...
        for (int j = 0; j < mins.length; j++) {
            ...
            if (paths[j][paths[j].length - 1] == dest) {
                ...
                while (i < paths[j].length) {
                    ...
                    if ((cur.val - w) > edgeFailCount) { // add by @Anan start
...
//                paths[j] = this.ra.getRoute(s, d, j, g, nodes, exclude, edgeweights, vals[j], edgeFailCount);
