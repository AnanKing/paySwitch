...
//失败链路达到N后进行调用
public class CreditFlare3 extends Metric {
    //input parameters
...
//0 8 0(不调用)
//    String routeSuccPath = "./data/Results/jan2013-lcc-t0-F-routeSucc-root1-dyn-NoPaySwitch-E8AllNonull.txt";
//    String paySuccPath = "./data/Results/jan2013-lcc-t0-F-paySucc-root1-dyn-NoPaySwitch-E8AllNonull.txt";
//    String edgeWeightPath = "./data/Results/jan2013-lcc-t0-F-finished-transactions-Weight-root1-dyn-NoPaySwitch-E8AllNonull.txt";
//    String routeFailPath = "./data/Results/jan2013-lcc-t0-F-routeFail-root1-dyn-NoPaySwitch-E8AllNonull.txt";
    ...
    public CreditFlare3(String file, String name, int rn, int k, int nb, int ntab,
    ...
    public CreditFlare3(String file, String name, int rn, int k, int nb, int ntab,
                        double epoch, Partitioner part, String links, boolean up, int edgeFailCount) {
        super("CREDIT_FLARE", new Parameter[]{new StringParameter("NAME", name), new IntParameter("REACH", rn),
                new IntParameter("ROUTES", k), new IntParameter("BEACONS", nb), new IntParameter("MAXNODES", ntab),
                new StringParameter("PARTITIONER", part.getName())});
        transactions = this.readList(file);
    ...
    public CreditFlare3(String file, String name, int rn, int k, int nb, int ntab,
                        double epoch, Partitioner part, boolean up, int edgeFailCount) {
        this(file, name, rn, k, nb, ntab, epoch, part, null, up, edgeFailCount);
    }
    ...
    public void computeData(Graph g, Network n, HashMap<String, Metric> m) {
        ...
        // int TransactionNumber = 0;// transaction number @add by anan
...
//            WriteToTxt.writeTxt(edgeWeightPath, s);
                ...
                for (int j = 0; j < paths.size(); j++) {
                    ...
                    for (int i = 1; i < p.length; i++) {
                        ...
                        if (!edgeweights.setWeight(l, k, vals[j])) {
                            ...
                            if (log) {
                                System.out.println("----Set weight of (" + l + "," + k + ") to " + edgeweights.getWeight(e) + "(previous" + w + ")");
                            }
                        }
                        l = k;
                    }
    ...
    public Single[] getSingles() {
        ...
        return new Single[]{m_av, m_S_av, m_F_av, p_av, p_S_av, p_F_av, d1, d2, d3,
                s_av, s, SAA, SCA, PAA, pP_av, pPF_av, pPNF_av, init};
    }
