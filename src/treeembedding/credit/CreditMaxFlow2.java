...
//每隔stepcount条交易调用一次的
public class CreditMaxFlow2 extends Metric {
    //input parameters
...
//    String routeFailPath = "X:\\out\\Mydataset-routeFail-t0-graph-root3-dyn-test21.txt";
    ...
    public CreditMaxFlow2(String file, String name, double requeueInt,
    ...
    public CreditMaxFlow2(String file, String name, double requeueInt,
                          int max, String links, boolean up, double epoch, int stepCount) {
        super("CREDIT_MAX_FLOW", new Parameter[]{
                new DoubleParameter("REQUEUE_INTERVAL", requeueInt),
                new IntParameter("MAX_TRIES", max)});
        transactions = this.readList(file);
    ...
    public CreditMaxFlow2(String file, String name, double requeueInt, int max,
                          boolean up, double epoch, int stepCount) {
        this(file, name, requeueInt, max, null, up, epoch, stepCount);
    }
    ...
    public CreditMaxFlow2(String file, String name, double requeueInt, int max,
                          String links, double epoch, int stepCount) {
        this(file, name, requeueInt, max, links, true, epoch, stepCount);
    }
    ...
    public void computeData(Graph g, Network n, HashMap<String, Metric> m) {
        ...
        while (c < this.transactions.size() || toRetry.size() > 0) {
            ...
            count++;
            //if (log){
            // System.out.println("The path is:"+pathAll);
            //  System.out.println("The path is: Test");
            System.out.println("Perform transaction s=" + cur.src + " d= " + cur.dst +
                    " val= " + cur.val + " time= " + cur.time);
            allTransaction++;
            System.out.println("Static network The current transaction number is: " + allTransaction);
            ...
            int[] results = this.fordFulkerson(cur, g, nodes, exclude, false);
            cur_count++;
            if (results[0] == 0) {
                cur_succ++;
                FailTimes++;// add by anan
                System.out.println("Fail result is 0 times is:" + FailTimes);// add by anan
            }
...
//                    toRetry.add(st, cur);
                ...
                topupNumbers++;// add by anan
                System.out.println("Fail result is -1 times is:" + topupNumbers);// add by anan
            }
            ...
            if (count % stepCount == 0) {
                while (failTrans.size() > 0) {
                    ...
                    //2: execute the transaction
                    results = this.fordFulkerson(cur, g, nodes, exclude, true);
                    cur_count++;
                    if (results[0] == 0) {
                        cur_succ++;
                        FailTimes++;// add by anan
...
//                    toRetry.add(st, cur);
    ...
    private int[] fordFulkerson(Transaction cur, Graph g, Node[] nodes, boolean[] exclude, boolean switchPay) {
        ...
        while (totalflow < cur.val &&
                (resp = findResidualFlow(edgeweights, g.getNodes(), src, dest)).length > 1) {
            ...
            for (int i = 0; i < respath.length - 1; i++) {
                ...
                if (switchPay) {
                    if (cur.val > a) {
                        SwitchPayment.switchPayment(respath[i], respath[i + 1], nodes, edgeweights, cur.val);
                    }
                    a = edgeweights.getPot(respath[i], respath[i + 1]);
                }
                if (a < min) {
                    min = a;
                }
    ...
    public Single[] getSingles() {
        ...
        return new Single[]{m_av, m_Re_av, m_S_av, m_F_av, p_av, p_Re_av, p_S_av, p_F_av,
                s1, s, pP_av, pPF_av, pPNF_av, SAA, SCA, PAA, FSAA, FSCA, FPAA};
    }
