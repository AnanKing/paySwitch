...
public class MultipleSpanningTree extends Transformation {
    ...
    public Graph transform(Graph graph) {
        String OGDPath = "X:\\project\\java\\speedymurmur-experiments\\speedy\\data\\OriginalGraphDegree-1.txt";
        ...
        if (this.rootSelector.equals("bfs") && edgeweights == null) {
            ...
            for (int i = 0; i < nodes.length; i++) {
                ...
                if (this.oneDSel == Direct.TWOPHASE) {
                    l = potParents(graph, nodes[i], Direct.NONE, edgeweights).length; // Not judging weight
                } else {
            ...
            for (int i = 0; i < this.trees; i++) {
                ...
                map.put(roots[i], new ParentChild(-1,
                        roots[i], 0));//add root nodes
                int[] out = potChildren(graph, nodes[roots[i]], this.oneDSel, edgeweights); // Judge the weight is greater than 0   //
                if (out.length == 0) {
                    out = potChildren(graph, nodes[roots[i]], Direct.EITHER, edgeweights);//Judge the weight is greater than 0
                    if (out.length == 0) {
                        out = potChildren(graph, nodes[roots[i]], Direct.NONE, edgeweights);//Not judging weight
                    }
                }
                for (int j = 0; j < out.length; j++) {
                    ...
                    vec.add(new int[]{roots[i], i, 1});// add parent node,children node, depth to vec
                }
            }//obtain the children nodes of nodes and add it into offers
            ...
            while (!offers.isEmpty()) {
                ...
                while (it.hasNext()) {
                    ...
                    int min = this.trees;
                    int[] out = this.potParents(graph, nodes[index], Direct.NONE, edgeweights);//Not judging weight
                    String pot = "";
                ...
                while (itnext.hasNext()) {
                    ...
                    map.put(index, new ParentChild(parent[0],
                            index, parent[2]));//add node 2 to parentChildMap
                    int[] out = potChildren(graph, nodes[index], this.oneDSel, edgeweights); //the oneDSel will change every loop. (not) judge weight    //
                    for (int j = 0; j < out.length; j++) {
                        if (map.containsKey(out[j])) continue;
                        int[] added = next.get(out[j]);
                        if (added == null || added[1] != parent[1]) {
                            int[] o = new int[]{index, parent[1], parent[2] + 1};// add depth of nodes
                ...
                if (offers.isEmpty() && this.oneDSel == Direct.TWOPHASE) {
                    ...
                    for (Node n : nodes) {
                        ...
                        int[] out = potChildren(graph, n, Direct.EITHER, edgeweights);// Judge the weight is greater than 0
                        for (int i = 0; i < this.trees; i++) {
                            HashMap<Integer, ParentChild> map = parentChildMap.get(i);// three parameters means are parent, child, depth. -1 means no parent nodes//
                ...
                if (offers.isEmpty()) {
                    ...
                    for (Node n : nodes) {
                        int index = n.getIndex();
                        int[] out = potChildren(graph, n, Direct.NONE, edgeweights);// Not judging weights
                        for (int i = 0; i < this.trees; i++) {
                            HashMap<Integer, ParentChild> map = parentChildMap.get(i);//three parameters means are parent, child, depth. -1 means no parent nodes
                            ParentChild parent = map.get(index);
                            if (parent == null) continue;
            ...
            // Write by Anan, it is aims to obtain the degree of graph and spanning tree.
            for (Node n : nodes) {
                int index = n.getIndex();
                int[] degree = nodes[index].getIncomingEdges();
                String Content = index + " " + degree.length;
                //WriteToTxt.writeTxt(OGDPath, Content);
            }
            ...
            CreditLinks.setParentChildMapDepths(parentChildMap);
        }
        ...
        if (ew != null && !dir.equals(Direct.NONE)) {
            ...
            for (int j = 0; j < in.length; j++) {
                if (ew.getPot(in[j], n.getIndex()) > 0) {
                    ...
                    //System.out.println("-------The IngoingEdges Balance (" + in[j] + "," + n.getIndex() + ") is:" + ew.getPot(in[j], n.getIndex()) + "------");// Anan 319-322
//                    if (Math.abs(ew.getWeight(in[j], n.getIndex())) > 0) {
//                        System.out.println("-------The IngoingEdges weight (" + in[j] + "," + n.getIndex() + ") is:" + ew.getWeight(in[j], n.getIndex()) + "------");// Anan 319-322
//                        creditIn.add(in[j]);
//                    }
                    creditIn.add(in[j]);//Removed by Anan
...
//						}
                ...
                if (ew.getPot(n.getIndex(), out[j]) > 0) {
                    ...
                    // System.out.println("-------The OutgoingEdges Balance (" + n.getIndex() + "," + out[j] + ") is:" + ew.getPot(n.getIndex(), out[j]) + "------");// Anan329-332
//                    if (Math.abs(ew.getWeight(n.getIndex(), out[j])) > 0) {
//                        System.out.println("-------The OutgoingEdges weight (" + n.getIndex() + "," + out[j] + ") is:" + ew.getWeight(n.getIndex(), out[j]) + "------");// Anan329-332
//                        creditOut.add(out[j]);
//                    }
                    creditOut.add(out[j]); //Removed by Anan
...
//						}
            ...
            for (int i = 0; i < in.length; i++) {
                in[i] = creditIn.get(i);
                //System.out.println("The in edges is" + in[i] + "-------------");
            }
            out = new int[creditOut.size()];
            for (int i = 0; i < out.length; i++) {
                out[i] = creditOut.get(i);
                //System.out.println("The out edges is" + out[i] + "----------------");
            }
        }
        switch (dir) {
            case UP:
